...
- **當 $\omega \ne 0$ 時**，由於 $\zeta(s)$ 在臨界線邊界 $\mathrm{Re}(s) = 1$ 上無極點（唯一極點在 $s=1$），且由 Hadamard-de la Vallée Poussin 定理，$\zeta(1 - i\omega) \ne 0$（邊界無零點）；
- 因此，在 $\mathrm{Re}(s) = 1$ 上**不存在任何主極點**！由經典質數定理標準零點自由區（Zero-Free Region）界：
...
1. **主階頻率振盪項（第 1 項）**：$\frac{1}{2}\mathrm{Im}\left(-\frac{\zeta'}{\zeta}(1/2 - 2it; X)\right)$（由 Dirichlet 級數恆等式確證）；
...
2. **非平凡頻譜調製唯一性**：不同頻率間的相對增長差異 $\log R(X, t_1) - \log R(X, t_2) = \frac{1}{2}\mathrm{Im}(-\zeta'/\zeta(1/2-2it_1)) - \frac{1}{2}\mathrm{Im}(-\zeta'/\zeta(1/2-2it_2)) + \mathcal{O}_{t_1, t_2}(X)$，**100% 唯一由臨界線上的質數 Dirichlet 多項式 $-\frac{\zeta'}{\zeta}(1/2 - 2it; X)$ 統御**！
