...
- **退化集合之零測度性**：
  ...
  - 依據 Borel-Cantelli 引理，在頻率軸上幾乎處處（almost everywhere）滿足 $|\cos\alpha_X(t)| \ge \mathcal{O}_t(1)$，從而：
    $$\mathbf{\log R_1(X, t) = \log s_1(X, t) + \mathcal{O}_t(1) = \frac{1}{16}X^2 + \frac{1}{2}\mathrm{Im}S(X, t) + \mathcal{O}_t(X) \quad (\text{a.e. } t \in \mathbb{R})}$$
...
因此幾乎處處（a.e. $t$）滿足 $\log R_1(X, t) = \log s_1(X, t) + \mathcal{O}_t(1) = \frac{1}{16}X^2 + \frac{1}{2}\mathrm{Im}S(X, t) + \mathcal{O}_t(X)$。
...
- **Oseledets 奇異向量非退化性之 Lebesgue 測度滿秩引理（Theorem 381.2）**：
  - 由阿基米德相角速度 $\frac{\partial\alpha_X}{\partial t} \ge c_0 X > 0$，證明退化集合 $\mathcal{E}_X(c) = \{t : |\cos\alpha_X| \le e^{-c X^2}\}$ 的 Lebesgue 測度滿足 $\mathrm{Leb}(\mathcal{E}_X) \le \frac{C}{X} e^{-c X^2} \to 0$；
...
2. **Oseledets 測度滿秩引理**：定理 381.2 透過相角速度 $\frac{\partial\alpha_X}{\partial t} \ge c_0 X$ 證明退化集合測度 $\mathrm{Leb}(\mathcal{E}_X) \le \frac{C}{X}e^{-c X^2} \to 0$，是否 100% 嚴密封閉了上一輪指出的非退化性缺口？
