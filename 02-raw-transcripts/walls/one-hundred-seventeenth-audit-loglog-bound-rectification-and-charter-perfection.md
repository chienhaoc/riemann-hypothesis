...
**精確核算**：
由 Abel 分部求和積分公式，當 $|\operatorname{Im}S(u, t)| \le C_t u$ 時，截斷尺度 $X$ 上的界為：
...
在第一百一十五輪審查中，紅隊專家嚴正指出：Abel 分部積分恆等式 $\mathcal{S}_{\text{Selberg}}(X, t) = -\frac{\operatorname{Im}S(X, t)}{X} - \int_2^X \frac{\operatorname{Im}S(u, t)}{u^2} du$ 本身正確，但代入去卷積尺度 $X_t = \log(t/2\pi e)$ 後，所得量級為 $C_t \log(X_t) = C_t \log\log(t/2\pi e) \in \mathcal{O}(\log\log t)$，而非聲稱的 $\mathcal{O}(\log t)$。
...
由 Abel 分部積分公式，若 $|\operatorname{Im}S(u, t)| \le C_t u$，則對任意對數尺度 $X$：
$$|\mathcal{S}_{\text{Selberg}}(X, t)| \le C_t \log X + \mathcal{O}_t(1)$$
