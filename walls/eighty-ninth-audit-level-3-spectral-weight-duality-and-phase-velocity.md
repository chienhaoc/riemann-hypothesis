...
| **路線 B：自伴譜測度**| $w_k = 1/(\partial\phi/\partial t)(\infty, \lambda_k)$ | Prüfer 相速受控於振幅 $\log R \sim \frac{1}{16}X^2 + \frac{1}{2}\operatorname{Im}S(X, t)$|
...
- 譜權重 $w_k$ 的漸近行為取決於相速 $\frac{\partial\phi}{\partial t}$，而相速由 Prüfer 振幅 $\log R(X, t) = \frac{1}{16}X^2 + \frac{1}{2}\operatorname{Im}S(X, t) + \dots$ 決定；
