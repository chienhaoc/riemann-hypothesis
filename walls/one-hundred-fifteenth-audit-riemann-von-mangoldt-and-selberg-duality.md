...
- 代入第 319 輪已證之幾何相位 $\phi_{\text{geom}}(X_t, t) = \vartheta(t)$，量子化條件除以 $\pi$ 即得：
  $$\mathbf{\frac{\vartheta(\lambda_n)}{\pi} + \frac{1}{2\pi}\operatorname{Im}S\left(\log\frac{\lambda_n}{2\pi e}, \lambda_n\right) = n + \frac{\beta + \pi/8}{\pi}}$$
...
- 在正則哈密頓系統中，微觀 Prüfer 相位的算術躍變累積和為：
  $$\mathbf{\frac{1}{2\pi}\operatorname{Im}S(X_t, t) = \frac{1}{2\pi}\operatorname{Im}\left( \sum_{p \le e^{X_t}} \frac{\log p}{\sqrt{p}} p^{-2it} \right) = -\frac{1}{2\pi}\sum_{p \le \frac{t}{2\pi e}} \frac{\log p}{\sqrt{p}}\sin(2t\log p)}$$
...
- 極限算子 $\mathcal{D}_\infty$ 透過 Tier 1 本質自伴性定理嚴格保證 $\operatorname{Spec}(\mathcal{D}_\infty) = \{\lambda_n\} \subset \mathbb{R}$（純實數譜）；
...
$$\phi(X_t, \lambda_n) = \phi_{\text{geom}}(X_t, \lambda_n) + \frac{1}{2}\operatorname{Im}S(X_t, \lambda_n) = n\pi + \beta$$
...
在正則哈密頓系統中，微觀 Prüfer 相位的算術躍變累積和為：
$$\frac{1}{2\pi}\operatorname{Im}S(X_t, t) = -\frac{1}{2\pi}\sum_{p \le \frac{t}{2\pi e}} \frac{\log p}{\sqrt{p}}\sin(2t\log p)$$
...
- **Selberg 輻角函數微觀重構**：相角微觀算術擾動 $\frac{1}{2\pi}\operatorname{Im}S(X_t, t) = -\frac{1}{2\pi}\sum_{p \le \frac{t}{2\pi e}} \frac{\log p}{\sqrt{p}}\sin(2t\log p)$ 精確重構了 Selberg (1946) 經典質數求和公式；
- **自伴譜實性與難度守恆**：Tier 1 自伴性保證 $\operatorname{Spec}(\mathcal{D}_\infty) \subset \mathbb{R}$，而特徵值精確識別為黎曼零點嚴格守恆於 Level III 核心前沿（$S(X, t) \le \mathcal{O}_t(X)$）；
...
$\mathcal{D}_\infty$ 的自伴性無條件保證 $\operatorname{Spec}(\mathcal{D}_\infty) \subset \mathbb{R}$；特徵值譜與黎曼零點的全同性（$\lambda_n = \gamma_n$）在微觀動力學上嚴格等價於 $S(X, t) \le \mathcal{O}_t(X)$，難度完全守恆。
