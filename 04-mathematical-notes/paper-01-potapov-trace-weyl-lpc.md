...
*For any $z \in \mathbb{C}^+ = \{z \in \mathbb{C} : \operatorname{Im}(z) > 0\}$, the fundamental solution matrix $\mathcal{Y}(u, z)$ satisfying $\mathcal{Y}(0, z) = I_2$ obeys the Potapov differential identity:*
$$\frac{d}{du}\left(\mathcal{Y}^*(u, z)(-iJ)\mathcal{Y}(u, z)\right) = 2\operatorname{Im}(z)\mathcal{Y}^*(u, z)H(u)\mathcal{Y}(u, z) \succeq 0$$
*where $H(u) = -J V(u) \succeq 0$. Furthermore, the matrix trace satisfies:*
$$\operatorname{tr}(\mathcal{Y}^*(u, z)\mathcal{Y}(u, z)) \ge 2 \quad (\forall u \ge 0)$$

*Proof*. Since $\det \mathcal{Y}(u, z) \equiv 1$ by Liouville's theorem ($\operatorname{tr}(J^{-1}V) = 0$), the eigenvalues of $\mathcal{Y}^*\mathcal{Y}$ are $\lambda_1, 1/\lambda_1$ with $\lambda_1 \ge 1$. Thus $\operatorname{tr}(\mathcal{Y}^*\mathcal{Y}) = \lambda_1 + 1/\lambda_1 \ge 2$. By Grönwall amplification, $\lambda_1(u) \ge e^{c u}$, guaranteeing divergence as $u \to \infty$. $\blacksquare$
...
$$R(u, z) = \frac{1}{2\operatorname{Im}(z)\int_0^u \|\mathbf{y}_1(s, z)\|^2 ds} \le \frac{1}{2u\operatorname{Im}(z)} \xrightarrow{u\to\infty} 0$$
